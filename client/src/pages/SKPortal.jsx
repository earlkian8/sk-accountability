...
export function SKPortal({ onBack }) {
  const { userRole, setUserRole, selectedBarangayId, addProgram } = useAppStore()
  ...
  const handleSubmit = async e => {
    e.preventDefault(); if (userRole !== 'sk-official') return
    setSubmitting(true)
    try {
      let photoUrl = null
      if (photoFile) photoUrl = await uploadPhoto(photoFile)
      const program = await createProgram({ ...form, budget: parseInt(form.budget), barangayId: selectedBarangayId, photoUrl })
      addProgram(program); setSubmitted(true)
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--gray-50)' }}>
      ...
      {userRole !== 'sk-official' && (
        <div style={{ background: 'var(--yellow-light)', borderBottom: '1px solid #fde68a', padding: '9px 24px', textAlign: 'center', fontSize: 13, fontWeight: 600, color: '#92400e', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }}>
          ⚠ Para makapag-sumite ng programa, palitan ang role sa <strong>SK Official</strong>
        </div>
      )}

      <main style={{ maxWidth: 680, margin: '0 auto', padding: '28px 24px', display: 'flex', flexDirection: 'column', gap: 18 }}>

        <div className="slide-up">
          <p className="label-caps" style={{ marginBottom: 4 }}>SK Official Portal</p>
          <h1 className="heading" style={{ fontSize: 28, color: 'var(--gray-900)' }}>Isumite ang Programa</h1>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Program info */}
          <div className="surface slide-up d1" style={{ padding: 24 }}>
            <p className="label-caps" style={{ marginBottom: 20 }}>Impormasyon ng Programa</p>
          ...
          <div className="surface slide-up d2" style={{ padding: 24 }}>
            <p className="label-caps" style={{ marginBottom: 16 }}>Larawan / Katibayan</p>
            {photoPreview ? (
              ...
              <label style={{
                ...
                border: '2px dashed var(--gray-200)', borderRadius: 'var(--radius)', padding: '36px 24px',
                ...
                <div style={{ width: 44, height: 44, borderRadius: 10, background: 'var(--blue-pale)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
          ...
          <button type="submit" disabled={userRole !== 'sk-official' || submitting}
